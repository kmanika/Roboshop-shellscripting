...
SETUP_SYSTEMD() {
  ...
  sed -i -e "s/MONGO_DNSNAME/mongodb.devops.jcp/" -e "s/REDIS_ENDPOINT/redis.devops.jcp/" -e "s/MONGO_ENDPOINT/mongodb.devops.jcp/" -e "s/CATALOGUE_ENDPOINT/catalogue.devops.jcp/" -e 's/CARTENDPOINT/cart.devops.jcp/' -e 's/DBHOST/mysql.devops.jcp/' -e 's/CARTHOST/cart.devops.jcp/' -e 's/USERHOST/user.devops.jcp/' -e 's/AMQPHOST/rabbitmq.devops.jcp/'  /home/roboshop/${COMPONENT}/systemd.service && mv /home/roboshop/${COMPONENT}/systemd.service /etc/systemd/system/${COMPONENT}.service
  STAT_CHECK $?
